...
export function buildUserPayload(user) {
  ...
  return {
    ...
    savedAssignments: (user.assignments || []).map((assignment) => ({
      ...
      result: safeParseJson(assignment.result),
...
export async function deleteSavedAssignment(userId, assignmentId) {
...
export async function upgradeToPremium(userId, billing = {}) {
  return prisma.user.update({
    where: { id: userId },
    data: {
      premium: true,
      premiumActivatedAt: new Date(),
      lemonSqueezyCustomerId: billing.customerId ?? undefined,
      lemonSqueezyOrderId: billing.orderId ?? undefined,
      lemonSqueezySubscriptionId: billing.subscriptionId ?? undefined,
    },
  });
}

export function buildSessionCookie(token) {
  return `sessionToken=${token}; ${getCookieOptions(7 * 24 * 60 * 60)}`;
}

export function clearSessionCookie() {
  return `sessionToken=; ${getCookieOptions(0)}`;
}
